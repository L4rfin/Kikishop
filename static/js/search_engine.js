...
function generate_pages() {
    let container = document.getElementById('list-of-page');
    let button_container = document.createElement('div');
    button_container.id = 'button-container'
    container.innerHTML = ''
    let h2 = document.createElement('h2');
    const buttonsCount = Math.ceil(amounts / 10);
    ...
    if (amounts <= 10) {
        h2.innerText = ++selected_page + '-' + (amounts) + ' fo ' + amounts;
    }
    if (amounts > 10) {
        h2.innerText = selected_page * 10 + 1 + '-' + Math.max(Math.min(selected_page * 20, amounts), 10) + ' of ' + amounts
    }
